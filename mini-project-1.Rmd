...
<!-------------------------- Start your work below ---------------------------->
### Answer to #1. 
There are many numeric variables to plot. I can plot the distribution of apartment buildings with different #s of floors, units, or buildings constructed by decade.
Here, I decided to see what the distribution of buildings built by year.
```{r}
year_built_hist<- ggplot(apt_buildings, aes(year_built)) +
  geom_histogram()
print(year_built_hist)
```
The # buildings built over time seems to follow a normal distribution, with most buildings being built in ~the 1960s. 

### Answer to #4

I am interested to see if buildings built in a certain year will have certain amenities such as an elevator or be labelled non-smoking. Exploring this relationship relates to my first research question and is important to investigate. Q: Does the age of the building affect the # of non-smoking buildings
```{r}
#
...
### Answer to #5
I could filter buildings that fit a certain criteria (ex, features I would 'NEED' in an apartment) to see how many units fit that description.
```{r}
...
### Answer to #7
I could easily subset for apartment qualities I am interested in and subtract redundant columns (ex, columns where every apartment fulfills the requirement such as hot water + fire alarms). This will help me look at the differences in the data set rather than obvious similarities. Since I already filtered for my requirements in an apartment, I want to see the listing # and address info for the available units.
